...
@dataclass
class Trivia:
    f: Callable[[str, dict[str, Any]], float]
    description: Callable[[str, float], str]
    fun_factor: Callable[[float], float]
...
TRIVIAS = [
    ...
    Trivia(
        f=lambda p, json: to_stats(json)["resourceStats"][p]["tradeIncome"] - to_stats(json)["resourceStats"][p]["tradeLoss"],
